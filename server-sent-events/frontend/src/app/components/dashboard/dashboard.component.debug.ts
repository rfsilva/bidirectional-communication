...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    console.log('🚀 Dashboard: Inicializando com debug...');
  ...
  private waitForAuthenticationReady(): void {
    console.log('⏳ Dashboard: Aguardando autenticação estar completamente pronta...');
    
    // Aguardar que a autenticação esteja completamente pronta
    this.authService.waitForAuthReady().subscribe(() => {
      console.log('✅ Dashboard: Autenticação completamente pronta - inicializando recursos protegidos');
      this.authReady = true;
      this.currentUser = this.authService.getCurrentUser();
      
      // DEBUG: Testar autenticação antes de carregar dados
      this.runAuthenticationDebug();
      
      this.initializeProtectedResources();
    });

    // Também observar mudanças no estado de autenticação
    this.subscriptions.push(
      this.authService.authReady$.subscribe(ready => {
        if (ready && !this.authReady) {
          console.log('✅ Dashboard: Autenticação ficou pronta - inicializando recursos protegidos');
          this.authReady = true;
          this.currentUser = this.authService.getCurrentUser();
          
          // DEBUG: Testar autenticação
          this.runAuthenticationDebug();
          
          this.initializeProtectedResources();
        } else if (!ready && this.authReady) {
          console.log('🔒 Dashboard: Autenticação perdida - desconectando recursos protegidos');
  ...
  private runAuthenticationDebug(): void {
    console.log('🧪 Dashboard: Executando debug de autenticação...');
    
    // Aguardar um pouco para garantir que tudo está pronto
    setTimeout(() => {
      this.authDebugService.testTokenInRequests();
      this.authDebugService.startRequestMonitoring();
    }, 500);
  }
  ...
  private initializeProtectedResources(): void {
    ...
    // Verificar novamente se está tudo pronto
    if (!this.authService.isAuthenticated() || !this.authService.getToken()) {
      console.warn('🔒 Dashboard: Autenticação não está pronta, aguardando mais um pouco...');
      setTimeout(() => this.initializeProtectedResources(), 500);
      return;
    }
    
    console.log('✅ Dashboard: Tudo pronto, carregando dados e conectando SSE');
  ...
  private initializeSSE(): void {
    if (!this.authReady || !this.authService.isAuthenticated()) {
      console.warn('🔒 Dashboard: Não é possível inicializar SSE - autenticação não pronta');
      return;
    }
  ...
  private setupSubscriptions(): void {
    ...
    this.subscriptions.push(
      this.authService.currentUser$.subscribe(user => {
        ...
        console.log('👤 Dashboard: Usuário atual:', user?.username || 'nenhum');
      })
    );
  }
  ...
  private loadInitialData(): void {
    if (!this.isAuthenticationReady()) {
      console.warn('🔒 Dashboard: Autenticação não está pronta para carregar dados');
      return;
    }
  ...
  loadAllData(): void {
    ...
    console.log('🔍 Dashboard: Estado antes da requisição:', {
      isAuthenticated: this.authService.isAuthenticated(),
      hasToken: !!this.authService.getToken(),
      tokenPreview: this.authService.getToken()?.substring(0, 20) + '...'
    });
    ...
    this.dataService.getAllData().subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 401) {
          console.error('🔒 Dashboard: Erro 401 - executando debug...');
          this.authDebugService.testTokenInRequests();
  ...
  loadExternalData(): void {
    ...
    this.dataService.getExternalData().subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 401) {
          console.error('🔒 Dashboard: Erro 401 em dados externos - executando debug...');
          this.authDebugService.testTokenInRequests();
        }
      }
    });
  ...
  loadInternalData(): void {
    ...
    this.dataService.getInternalData().subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 401) {
          console.error('🔒 Dashboard: Erro 401 em dados internos - executando debug...');
          this.authDebugService.testTokenInRequests();
        }
      }
    });
  ...
  loadStats(): void {
    ...
    this.dataService.getStats().subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 401) {
          console.error('🔒 Dashboard: Erro 401 em estatísticas - executando debug...');
          this.authDebugService.testTokenInRequests();
        }
      }
    });
  ...
  private isAuthenticationReady(): boolean {
    ...
    const result = isAuth && hasToken && hasUser && ready;
    
    if (!result) {
      console.warn('🔒 Dashboard: Autenticação não está pronta:', {
        isAuth,
        hasToken,
        hasUser,
        ready,
  ...
  // Método de debug para testar todos os endpoints
  testAllEndpoints(): void {
    console.log('🧪 Dashboard: Testando todos os endpoints protegidos...');
    this.authDebugService.testAllProtectedEndpoints();
  }
  ...
  createNewItem(): void {
    ...
    this.dataService.createData(this.newItem).subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 401) {
          console.error('🔒 Dashboard: Erro 401 ao criar item - executando debug...');
          this.authDebugService.testTokenInRequests();
    ...
    if (confirm('Tem certeza que deseja excluir este item?')) {
      ...
      this.dataService.deleteData(id).subscribe({
        ...
        error: (error) => {
          ...
          if (error.status === 401) {
            console.error('🔒 Dashboard: Erro 401 ao excluir item - executando debug...');
            this.authDebugService.testTokenInRequests();
          } else if (error.status === 403) {
            alert('Você não tem permissão para excluir dados');
          } else {
  ...
  forceExternalFetch(): void {
    ...
    this.notificationService.forceExternalDataFetch().subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 401) {
          console.error('🔒 Dashboard: Erro 401 na busca forçada - executando debug...');
          this.authDebugService.testTokenInRequests();
        } else if (error.status === 403) {
          alert('Você não tem permissão para esta ação');
        }
  ...
  sendTestNotification(): void {
    ...
    this.notificationService.sendTestNotification(message).subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 401) {
          console.error('🔒 Dashboard: Erro 401 na notificação - executando debug...');
          this.authDebugService.testTokenInRequests();
        }
      }
    });
