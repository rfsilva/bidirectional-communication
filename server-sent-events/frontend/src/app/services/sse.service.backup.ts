...
export class SSEService {
  ...
  connect(): void {
    ...
    if (!this.authService.isAuthenticated()) {
      console.warn('🔒 SSE: Usuário não autenticado, conexão rejeitada');
      console.warn('🔍 SSE: Status de autenticação:', {
    ...
    if (!token) {
      console.error('🔒 SSE: Token JWT não encontrado, conexão impossível');
