    ...
    if (req.url.includes('/api/')) {
      ...
      return this.authService.waitForInitialization().pipe(
        ...
        switchMap(() => {
          ...
          if (token) {
            console.log('🔐 JwtInterceptor: Token encontrado após inicialização, adicionando à requisição');
            ...
            // Verificar se há token no localStorage diretamente
            const localToken = localStorage.getItem('sse-demo-token');
            console.error('🔍 JwtInterceptor: Token no localStorage:', localToken ? 'Presente' : 'Ausente');
