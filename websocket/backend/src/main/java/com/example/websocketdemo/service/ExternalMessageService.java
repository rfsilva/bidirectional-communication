 ...
 * Atualizado para usar WebSocket ao invés de SSE.
 */
@Service
...
public class ExternalMessageService {
    ...
    private final WebSocketNotificationService webSocketNotificationService; // Mudança: WebSocket ao invés de SSE
    ...
    /**
     * Executa busca periódica de mensagens externas a cada 20 segundos.
     */
    ...
    public List<UserMessage> simulateExternalAPICall() {
        ...
        for (int i = 0; i < messageCount; i++) {
            try {
                ...
                // 🆕 ENVIAR NOTIFICAÇÃO WEBSOCKET INDIVIDUAL PARA O USUÁRIO
    ...
    /**
     * 🆕 Envia notificação WebSocket individual para o usuário que recebeu uma nova mensagem.
     */
    public void sendMessageNotificationToUser(UserMessage userMessage) {
        try {
            User user = userMessage.getUser();
            
            // Verificar se o usuário tem conexões WebSocket ativas
            boolean isUserConnected = webSocketNotificationService.isUserConnected(user.getId());
            
            if (isUserConnected) {
                // Enviar notificação WebSocket individual
                webSocketNotificationService.sendNewMessageNotification(
                    user.getId(),
                ...
                log.info("🔔 Notificação WebSocket enviada para usuário {} (ID: {})", 
                        user.getUsername(), user.getId());
            } else {
                log.debug("📱 Usuário {} (ID: {}) não tem conexões WebSocket ativas - notificação não enviada", 
                         user.getUsername(), user.getId());
            }
            
        } catch (Exception e) {
            log.error("❌ Erro ao enviar notificação WebSocket para mensagem ID {}: {}", 
     ...
     */
    public List<UserMessage> simulateExternalMessageFetch() {
     ...
     */
    public String personalizeMessageContent(String template, User user) {
    ...
    public UserMessage createTestMessage(Long userId, String source) {
        ...
        // 🆕 Enviar notificação WebSocket para o teste também
        sendMessageNotificationToUser(userMessage);
    ...
    /**
     * 🆕 Cria uma mensagem de teste personalizada e envia notificação WebSocket.
     */
     ...
     */
    public MessageTemplate getTemplateBySource(String source) {
    ...
    /**
     * 🆕 Obtém estatísticas das notificações WebSocket enviadas.
     */
    public Map<String, Object> getNotificationStats() {
        return Map.of(
            "totalActiveConnections", webSocketNotificationService.getActiveConnectionsCount(),
            "activeUsers", webSocketNotificationService.getActiveUsersCount(),
            "connectionStats", webSocketNotificationService.getConnectionStats(),
    ...
    public static class MessageTemplate {
        ...
        public final UserMessage.MessageType type;
        public final UserMessage.Priority priority;

        public MessageTemplate(String source, String title, String content,
                        UserMessage.MessageType type, UserMessage.Priority priority) {
            this.source = source;
            this.title = title;
