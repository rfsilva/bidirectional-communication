...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    this.webSocketService.disconnect(); // Mudança: WebSocket ao invés de SSE
  }
  ...
  private initializeWebSocket(): void { // Mudança: WebSocket ao invés de SSE
    if (!this.authReady || !this.authService.isAuthenticated()) {
      return;
    }

    this.updateDebugInfo();
    this.webSocketService.connect(); // Mudança: WebSocket ao invés de SSE
  }

  private updateDebugInfo(): void {
    this.debugInfo = this.webSocketService.getDebugInfo(); // Mudança: WebSocket ao invés de SSE
  }

  private setupSubscriptions(): void {
    // Monitor WebSocket connection status (Mudança: WebSocket ao invés de SSE)
    this.subscriptions.push(
    ...
    // Monitor WebSocket notifications (gerais) (Mudança: WebSocket ao invés de SSE)
    this.subscriptions.push(
      this.webSocketService.notifications$.subscribe(notification => {
        if (notification) {
          this.handleWebSocketNotification(notification); // Mudança: WebSocket ao invés de SSE
        }
      })
    );
    ...
    this.subscriptions.push(
      this.webSocketService.messageNotifications$.subscribe(notification => {
        if (notification) {
  ...
  private handleWebSocketNotification(notification: NotificationMessage): void { // Mudança: WebSocket ao invés de SSE
  ...
  clearMessageNotifications(): void {
    ...
    this.unreadMessageCount = 0;
    this.webSocketService.clearMessageNotifications(); // Mudança: WebSocket ao invés de SSE
  ...
  forceExternalFetch(): void {
    if (!this.canManageWebSocket()) { // Mudança: WebSocket ao invés de SSE
  ...
  // CORRIGIDO: Método para reconectar WebSocket
  ...
  // CORRIGIDO: Método para verificar permissão de gerenciar WebSocket
  canManageWebSocket(): boolean {
    return this.currentUser ? PermissionUtils.hasPermission(this.currentUser.role, 'canManageSSE') : false;
  }

  // CORRIGIDO: Alias para compatibilidade com template
  canManageSSE(): boolean {
    return this.canManageWebSocket();
  }
